<template>
    <div>
        <select class="picker" ref="pfx" :value="value" @input="updatePrefix()">
            <option disabled value="">Country code</option>
            <option v-for="pfx in prefixes" :key="pfx[1]" v-bind:value="pfx[0]">{{pfx[1]}}</option>
        </select>
    </div>
</template>
...
<style scoped>
    .picker {
        ...
        background-position: calc(100% - 9px) calc(1.0em + 3px),calc(100% - 4px) calc(0.5em + 8px),calc(100% - 1.5em) .5em;
